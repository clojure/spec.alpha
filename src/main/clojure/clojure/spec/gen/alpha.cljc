...
(ns clojure.spec.gen.alpha
    (:refer-clojure :exclude [boolean bytes cat hash-map list map not-empty set vector
                              char double int keyword symbol string uuid delay shuffle #?(:clje tuple)]))
   ...
   (defn- dynaload
     ...
     (let [ns (namespace s)]
       ...
       (let [v (resolve s)]
         (if v
           @v
           (throw (clojerl.Error. (str "Var " s " is not on the classpath"))))))))

(def ^:private quick-check-ref
  #?(:clj (c/delay (dynaload 'clojure.test.check/quick-check))
     :clje (dynaload 'clojure.test.check/quick-check)))
(defn quick-check
  [& args]
  (apply @quick-check-ref args))

(def ^:private for-all*-ref
  #?(:clj (c/delay (dynaload 'clojure.test.check.properties/for-all*))
     :clje (dynaload 'clojure.test.check.properties/for-all*)))
(defn for-all*
...
#?(:clj
   ...
   :clje
   (do
     (def ^:private g? (dynaload 'clojure.test.check.generators/generator?))
     (def ^:private g (dynaload 'clojure.test.check.generators/generate))
     (def ^:private mkg (dynaload 'clojure.test.check.generators/->Generator))
     (defn- generator?
       [x]
       (g? x))
     (defn- generator
...
(defn gen-for-name
  ...
  (let [g (dynaload s)]
    (if (generator? g)
      g
      (throw (#?(:clj RuntimeException. :clje clojerl.Error.)
              (str "Var " s " is not a generator"))))))
